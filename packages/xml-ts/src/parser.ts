...
import { AttributeValueSyntax } from "./enums";
import { isAttributeValueEnd, parseQualifiedName } from "./utilities";
...
export class SAXParser {
		...
		while (i < chunk.length) {
			...
			switch (this.state) {
				...
				case State.AttributeValueUnquoted: {
					if (!isAttributeValueEnd(charCode)) {
						if (charCode === Ascii.Ampersand) {
				 ...
				 */
				case State.Comment: {
					this.comment.value += char;

					if (this.comment.value.endsWith("-->")) {
						const comment = this.comment.value.slice(0, -3);
				...
				case State.OpenWaka: {
					...
					if (this.startTagPosition + 1 < this.position) {
						...
						this.textNode.value += " ".repeat(pad);
					}
				...
				case State.TextEntity: {
					...
					if (this.entity.length) {
						if (charCode === Ascii.Number || isNameCharacter(codePoint)) {
							this.entity += char;
							continue; // Move to next input character
						}
					} else {
						if (charCode === Ascii.Number || isNameStartCharacter(codePoint)) {
							this.entity += char;
							continue; // Move to next input character
						}
					}
	...
	private checkBufferLength(): void {
		...
		if (this.textNode.value.length > maxBufferLength) {
			this.flushText();
		}

		longestBufferLength = Math.max(
			longestBufferLength,
			this.textNode.value.length,
		);
	...
	private processCloseTag(): void {
		...
		if (index < 0) {
			...
			this.textNode.value += `</${this.tag.name}>`;
			this.state = State.Text;
			return;
		}
	...
	private processOpenTag(selfClosing = false): void {
		if (this.options.namespaces) {
			const { prefix, localName } = parseQualifiedName(this.tag.name, false);
			...
			this.attributeList.length = 0;
			this.namespaceStack.push(this.tag.ns);
		}
	...
	private resetTag(): void {
		this.tag.name = "";
		this.tag.attributes = {};
		this.tag.selfClosing = false;
	}

	private resetTextNode(): void {
		this.textNode.value = "";
	}
