...
export class SAXParser {
	...
	constructor(strict: boolean, options: SAXOptions = {}) {
		this.clearBuffers();
	...
	private emitNode(event, data): void {
		if (this.textNode) {
			this.closeText();
		}
	...
	private processAttribute(): void {
		...
		if (this.options.xmlns) {
			...
			if (prefix === "xmlns") {
				...
				if (
					localName === "xml" &&
					this.attributeValue !== XML_PREDEFINED_NAMESPACES.xml
				) {
					this.strictFail(
					...
					localName === "xmlns" &&
					this.attributeValue !== XML_PREDEFINED_NAMESPACES.xmlns
				) {
					this.strictFail(
					...
					this.tag.ns[localName] = this.attributeValue;
				}
			}
		...
		while (true) {
			...
			switch (this.state) {
				...
				case State.ProcessingInstructionData: {
					if (!this.piData && isWhitespace(character)) {
						continue;
					} else if (character === "?") {
						this.state = State.ProcessingInstructionEnding;
					} else {
						this.piData += character;
					}
					continue;
				}
